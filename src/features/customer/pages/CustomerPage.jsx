...
export default function CustomerPage() {
  ...
  const handleAddCustomer = async (e) => {
    ...
    try {
      ...
      console.error("Erro ao adicionar cliente:", err.response?.data || err);
  ...
  const handleEditCustomer = async () => {
    ...
    try {
      await api.put(
        `/api/establishments/${establishmentId}/clients/${selectedCustomer.clientDataId}`,
        updatedData
      );
      fetchCustomers(establishmentId);
      closeModal();
  ...
  const handleDeleteCustomer = async (clientDataId) => {
    ...
    if (window.confirm("Tem certeza que deseja apagar este cliente?")) {
      try {
        await api.delete(
          `/api/establishments/${establishmentId}/clients/${clientDataId}`
        );
        fetchCustomers(establishmentId);
      } catch (err) {
